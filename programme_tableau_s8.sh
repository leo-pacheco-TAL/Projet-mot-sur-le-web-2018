			...
			then # URL OK
				...
				if [[ $ENCODAGE == "UTF-8" ]]
					then
						...
						#---------------------------------------------------------------------------
						# reste à faire : contexte, bigramme, comptage occurrences (à faire par nous) dans DUMP
						#contextes
						egrep -C 1 $3 ./DUMP-TEXT/$compteurtableau-$compteur.txt > ./CONTEXES/$compteurtableau-$compteur-contextes.txt #la variable compteur tableau discrimine les langues
						#bigrammes
						egrep -o "\w+" ./DUMP-TEXT/$compteurtableau-$compteur.txt > b1;
						tail tail -n +2 b1 > b2; #b1 et b2 seront des fichiers temporaires
						paste b1 b2 | sort -fg | uniq -ic | sort -rn > ./DUMP-TEXT/$compteurtableau-$compteur-bigram.txt; #créer un fichier contentant nos bigrammes, comme un dossier bigramme par ex
