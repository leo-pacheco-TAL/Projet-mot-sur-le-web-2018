...
# $3= le troisième correspond aux différentes formes (une expression régulières) dans les différentes langues, soit le motif.
...
#on récupère pour chaque ligne l'encodage indiqué dans la balise charset, on transforme ces caractères en majuscule puis on supprime les caractères de fin de ligne. 
...
function minigrep(){
	perl ./minigrep/minigrepmultilingue.pl "UTF-8" ./DUMP-TEXT/dumps_indiv/$compteurtableau-$compteur.txt ./minigrep/motif-regexp.txt ;
...
function add_columns(){
	echo "<tr><td>$compteur</td><td>$code_sortie</td><td><a target=\"_blank\" href=\"$ligne\">$ligne</a></td><td><a target=\"_blank\" href=\"../PAGES-ASPIREES/$compteurtableau-$compteur.html\">page aspirée n° $compteur</a></td><td>$ENCODAGE</td><td><a target=\"_blank\" href=\"../DUMP-TEXT/dumps_indiv/$compteurtableau-$compteur.txt\">DUMP  n° $compteur</a></td><td><a href=\"../CONTEXTES/contextes_indiv/$compteurtableau-$compteur.txt\">CT $compteurtableau-$compteur</a></td><td><a target=\"_blank\" href=\"../CONTEXTES/$compteurtableau-$compteur.html\">CTh $compteurtableau-$compteur</a></td><td>$nbmotif</td><td><a href=\"../DUMP-TEXT/index-$compteurtableau-$compteur.txt\">Ind $compteurtableau-$compteur</a></td><td><a href=\"../DUMP-TEXT/bigramme-$compteurtableau-$compteur.txt\">Bigr $compteurtableau-$compteur</a></td></tr>";
}
#-------------------------------------------------------------------------------
#Concaténer les fichiers DUMP dans un seul fichier *** REMPLACÉE PAR LE SCRIPT PYTHON Py_Dump ***
...
#-------------------------------------------------------------------------------
#on réinitialise le tableau si l'on doit le lancer plusieurs fois
...
for fichier in $(ls $1) #pour chaque fichier dans le repertoire courant,
	...
	do
		#vérification du code http de la page web.
		check_code_http $1;
		if [[ $code_sortie == 200 ]]
			then
				echo -e "URL ok";
				...
				if [[ $ENCODAGE == "UTF-8" ]]
				#on vérifie si l'encodage initial est utf-8
					then
					#si oui, on applique nos traitements sur les urls.
						echo -e "ENCODAGE initial <$ENCODAGE> OK : on passe au traitement \n";
						#aspiration de l'URL
						traitement;
						add_columns >> $2;
					else
					#sinon, il faut chercher quel est l'encodage initial.
							...
							# On cherche l'encodage de la page en appliquant la commande file sur la page aspirée
								...
								if [[ $ENCODAGEFILE == "UTF-8" ]]
									then
										...
										if [[ ENCODAGE_RETOUR != "" ]]
										#si on test si l'encodage est connu de la commande iconv.
											then
											#si oui, on applique les traitements
												traitement_2;
												add_columns >> $2;
											else
											#sinon, on doit changer l'url. 
